...
class LBFPMechanismDriver(mech_linuxbridge.LinuxbridgeMechanismDriver):
    ...
    def get_vif_details(self, agent, context):
        if self.fp_info is not None and self.fp_info['active']:
            if 'bridge' in self.fp_info['supported_plugs']:
                ...
                socket = get_vif_vhostuser_socket(socket_prefix,
                                                  socket_dir,
                                                  context.current['id'])
                br_name = "brq" + context.network.current['id']
                ...
                details_copy[constants.VIF_VHOSTUSER_FP_PLUG_TYPE] = 'bridge'
                ...
                details_copy[portbindings.VIF_DETAILS_BRIDGE_NAME] = br_name
                return details_copy
